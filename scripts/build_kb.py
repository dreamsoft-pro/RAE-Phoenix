...
def upsert_points(client: QdrantClient,
                  ...
                  batch: int = 512) -> None:
    """
    Upsert z automatycznym fallbackiem:
    - próbujemy dodać dense + sparse (tf-idf) do Qdrant,
    - jeśli qdrant-client nie obsługuje 'sparse_vectors', lecimy dense-only.
    """
    ...
    for i in range(0, len(chunks), batch):
        ...
        # 1) podejście hybrydowe (dense + sparse) — może się wywalić na starszym kliencie
        try:
            pts = [
                PointStruct(
                    id=k,
            ...
            continue  # poszło OK, next batch
        except Exception as e:
            # Klient nie pozwala na sparse_vectors => fallback
            if isinstance(e, PydanticValidationError) or "sparse_vectors" in str(e).lower():
                print("[KB] WARN: qdrant-client nie obsługuje 'sparse_vectors' — przechodzę na dense-only.", file=sys.stderr)
            else:
                # jeżeli inny błąd, też spróbujmy dense-only
                print(f"[KB] WARN: upsert dense+sparse nie powiódł się: {e}. Próbuję dense-only.", file=sys.stderr)

        # 2) fallback: dense-only
        pts_dense = [
            PointStruct(
                id=k,
                vector={"dense_code": vecs_dense[idx]},
