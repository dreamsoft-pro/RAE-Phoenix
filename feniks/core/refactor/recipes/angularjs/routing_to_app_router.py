...
class RoutingToAppRouterRecipe(RefactorRecipe):
    ...
    def execute(
        ...
        try:
            ...
            for route_data in plan.metadata["routes"]:
                # Skip redirects (handled separately)
                if route_data["redirect_to"]:
                    continue
    ...
    def _assess_risks(self, routing: RoutingMetadata) -> List[str]:
        """Assess risks for routing migration."""
        risks = []
        ...
        for route in routing.routes:
            ...
            if param_count > 2:
                risks.append(f"Route {route.path} has {param_count} parameters")

            # Nested routes
            if route.parent:
                risks.append(f"Route {route.name} is nested under {route.parent}")

            # Optional parameters (ui-router)
            if '?' in route.path:
                risks.append(f"Route {route.path} has optional parameters")

        risks.extend([
            "Route guards need manual implementation",
            "Resolve functions need conversion to data fetching",
            "Query parameters handling needs verification"
        ])

        return risks

    def _calculate_risk_level(self, routing: RoutingMetadata) -> RefactorRisk:
        """Calculate overall risk level."""
        nested_count = sum(1 for r in routing.routes if r.parent)
        complex_count = sum(1 for r in routing.routes if r.path.count(':') > 2)

        if nested_count > 3 or complex_count > 0:
...
//
// TODO: Import and use migrated component
// TODO: Handle data fetching (replace resolve functions)

import React from 'react';

{params_interface}

export default function {component_name}({{ params }}: {component_name}Props) {{
  ...
  return (
    <div>
      ...
      {{/* TODO: Import and render migrated component */}}
    </div>
  );
}}
"""
