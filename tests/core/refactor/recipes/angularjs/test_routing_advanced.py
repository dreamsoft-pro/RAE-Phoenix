import pytest
from feniks.core.refactor.recipes.angularjs.routing_to_app_router import RoutingToAppRouterRecipe, RoutingMetadata, RouteDefinition, RouterType
...
def test_routing_layout_generation():
    ...
    routes = [
        ...
        {"path": "/parent/child", "name": "parent.child", "controller": "ChildCtrl", "template": "child.html", "parent": "parent"}
    ]
    
    plan = RefactorPlan(
        recipe_name="test", project_id="test", target_modules=[], target_files=[],
        rationale="test", risks=[], risk_level=RefactorRisk.LOW, estimated_changes=1, validation_steps=[],
...
def test_routing_page_data_fetching():
    ...
    route_data = {
        "path": "/dashboard",
        "controller": "DashCtrl",
        "resolve": {"stats": "stub", "profile": "stub"}
    }
