...
from feniks.core.refactor.recipes.angularjs.template_to_jsx import TemplateToJsxRecipe, TemplateMetadata, AngularHTMLParser
...
def test_template_parser_ng_model():
    ...
    def test_template_to_jsx_ng_model_conversion():
        ...
        chunk = Chunk(file_path="test.html", text=html, start_line=1, end_line=1, id="1", chunk_name="test", language="html")
        ...
        recipe._extract_template_metadata = MagicMock(return_value=TemplateMetadata(
            file_path="test.html", raw_html=html,
            interpolations=[], directives={"ng-model": ["search"]}, filters=[],
            controller_ref=None, complexity_score=1
        ))
        recipe._find_chunk_by_path = MagicMock(return_value=chunk)
        
        plan = RefactorPlan(
            recipe_name="angularjs.template-to-jsx", project_id="test",
            target_modules=[], target_files=["test.html"], rationale="test",
            risks=[], risk_level=RefactorRisk.LOW, estimated_changes=1, validation_steps=[],
